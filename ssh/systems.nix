{
  "lpm-server.feri.um.si" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIE0JjbyztsKY2N3Y7lhG4cvOybcuHZhf1YIw/LGR/nVR";
    port = 12022;
  };
  "bastion.lpm.feri.um.si" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIK1p6tEwRxVlTFWfb/pGbAp6jwKIXcD7qyvpYsGjL0W3";
  };
  "lpm.rwx.si" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAtiHVVTwv+mN+q/z/QBH0EtxaS+tHLxiaocoiyHmJdn";
  };
  "bastion" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIK1p6tEwRxVlTFWfb/pGbAp6jwKIXcD7qyvpYsGjL0W3";
  };
  "bastion.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIK1p6tEwRxVlTFWfb/pGbAp6jwKIXcD7qyvpYsGjL0W3";
  };
  "minimal.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINFR+kICN6q40fQAh3Qz02go6PxIQPVwIQsXFROBabkz";
  };
  "builder.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGWHNlVuViBUOe35zJNO4g+C9H/RDZ8LEGeUNdIy0jtN";
  };
  "proxy.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGAeD9V6NC2x1kX8JRAAxMotAAzLJwkVIp5NxXpTQZGY";
  };
  "prometheus.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHczCezO9MYp9a8CWq/6xI8s3bguAqoxuqYgPhoXEfqS";
  };
  "runner1.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDiQKJCUO8PHxaR2Q0AbeBaniODcFtSe310oasEIL6C/";
  };
  "runner2.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILub+nPv2n69XEDidopa01yuT6U9q53n8yMVzx63Upwa";
  };
  "ears.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICfp0u3dM4WfK2DLmh3JVGX4asaYyXjJqKzWYK53jS7d";
  };
  "sso-test.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJlDx9wxreroWOqxj4JiuvKsuPxKB2wFkEbzy+IMKvQ/";
  };
  "gateway.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPn6rWbPYE7lZHVJldIBie/hn1de/4o4dozGTl1SQ/Ea";
  };
  "student-mqtt.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGF+gn7tUyP80z6RcijMUem5Gym7viIToL+Q6/pdide0";
  };
  "kaze.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPlGeTDnI5LlckBK0Q+OYtjHBA6BWlNX5NulMj1ObNx/";
  };
  "collab.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHvY1B2Pewdv4Ot92PATXM/GG5en28RGDzOMtm8VNggK";
  };
  "collab-dev.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBLT+XpN1s87xczLMz1doTgF4EVfILFad36v/o+0MgD4";
  };
  "collab-pora.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEm2I0i92c3yrWZQsqE2woeEcc9GdXQBGQz1ZBVz3eU0";
  };
  "collab-rri.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIElC+YBrWW2ihn2/MZQJ3MALZaDTXrXCro0196FgQRay";
  };
  "collab-vr.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJZe0St5C2V336eFqqvUvHrvez4pNGZlmsLNOXq/jJh7";
  };
  "collab-rsasm.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIARR/P+Q2fz7Pf8ll5xwi6aRpvQRt/L+xSIoF745WjfU";
  };
  "collab-test.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPJxSNk/WRfh4hbmI3U//L6zIUxmiFym0e9wSj1yVoR0";
  };
  "collab-catalog-dev.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIG9KT4VVzmR9hQu5mR45GAbekTVuGexomPkCkXiMj87U";
  };
  "grades.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFRwz01f1Srf+4yW5afmD986a4JoeNr2rK0IV4HvSqbS";
  };
  "grades-dev.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJSpFrt/udk4bdD98H8aZK2m81QH+UOKs+CNKngY/9A0";
  };
  "login.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBdeB5rPe3sOeqZh43wEXor3Ny3G4WWtnF9VMW6wuX/p";
  };
  "login-dev.l" = {
    key ="ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIA/Rphb+ZjGz1gDwSXK6vgJ5tcg/Kh8DgFQ92M7bNRk2";
  };
  "gc.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBR8OU3TXgHmZ+zVccSNDTajx63Tn5T4+Hu7IX4anVD7";
  };
  "gc-dev.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIMDPKWYVJrFohEiarDker8M131g1Iono9TtmSI9BEr2q";
  };
  "pmd-catalog.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGy/1ArAsF/OTTe8mrYTgqE5GW2IheX/OGwLRWIUmPO/";
  };
  "catalog.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOzwRFnSoVnauDLuUhrUMB042R0chgVnFVx2ETECT2RY";
  };
  "catalog-dev.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIME4hIZHdTiTRsbIQ8crDxHTtkckFq+A3iF4G5Dda8bM";
  };
  "catalog-view.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBFr8ExHLDlAxJ6LsjLMfaPNWxBR0r420lCaOu5RXRsF";
  };
  "catalog-summer-schools.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIATmXHRJrzgVVqE2jfsQM2MJRy1eW9g2cAe8JEnv1ghI";
  };
  "catalog-mini-academy.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPaoY+sk7/CXIdYRUC0b/CeeYvNdncPAU/w2OVBapQtH";
  };
  "catalog-manage.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIG2sXPLrDPYasSCOiZ9+gqD+xk4m7ktjzl/K1yM+pZ3t";
  };
  "bioma.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIN0BPLzPl5TRejsLDME22k824mfyaBlEt9tK7BYSBgZg";
  };
  "feriusa.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINFUDOJSJgg3yYieHz0QeDoOZW4BX8PzmeCvpS6aDY7o";
  };
  "usatour.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIC7of1GBUiVzEIiEjBA2/yPj1MIXbJHCZqyvh2u9nA8G";
  };
  "ps.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHxFqBJfEHXRPmsfOScLzKCYFD9kls/tzSJSXzSrpdX3";
  };
  "school-project.l" = {
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKvI6gZo90mbaTDI44m6MiMIoTkFcGEgmHMFxykfifd2";
  };
}

